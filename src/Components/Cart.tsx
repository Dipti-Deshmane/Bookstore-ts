...
function Cart() {
  ...
  const fetchCartItems = async () => {
    try {
      ...
      const data: CartItem[] = Array.isArray(response.result)
        ? response.result
        : [];
      setCartItems(data);
    } catch (error) {
      ...
      <div className="cart-details">
        ...
        {cartItems.map((item) => (
          <div key={item._id} className="cart-item">
            <img
              src={item.product_id.bookImage || book1}
              alt={item.product_id.bookName}
            />
            <div className="item-details">
              ...
              <div className="quantity">
                <button
                  onClick={() =>
                    handleQuantityChange(item._id, item.quantityToBuy - 1)
                  }
                >
                  -
                </button>
